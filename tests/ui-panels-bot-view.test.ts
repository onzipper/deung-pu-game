...
describe("M4 workspace redesign (owner brief 2026-07-17) — active-plan selection + CTA action mapping", () => {
  ...
  test("botCtaButtonLabel: ไม่ busy → label เดิม, busy → กำลังเริ่ม.../กำลังหยุด... แยกตาม kind", () => {
    ...
    expect(botCtaButtonLabel(startCta, false)).toBe("เริ่มบอท");
    expect(botCtaButtonLabel(stopCta, false)).toBe("หยุดบอท");
    expect(botCtaButtonLabel(startCta, true)).toBe("กำลังเริ่ม…");
    expect(botCtaButtonLabel(stopCta, true)).toBe("กำลังหยุด…");
  });
});
