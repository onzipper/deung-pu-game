  ...
  test("ผู้เล่นล่อออกนอก pocket แล้วหนีไกล → มอนเลิก aggro → return → กลับเข้า pocket", () => {
    const sim = createMobSimulation({
      ...
      config: makeConfig({
        ...
        ai: { aggroRadius: { slime: 8 }, chaseSpeed: 3, leashRadius: { slime: 12 }, deaggroRadius: 6 },
        lod: { aoiRadius: 30 },
      }),
      hpFor: HP_FOR,
      rng: createLcgRng(4),
...
describe("attack — มอน chase เข้าระยะ → contact ผู้เล่น (A1, COMBAT_BIBLE §4/§7)", () => {
  ...
  test("player หนีออกนอกระยะ (leash) → เลิก aggro, ไม่มี contact", () => {
    const sim = createMobSimulation({
      map: makeMap(1, 1),
      config: makeConfig({
        ai: { aggroRadius: { slime: 3 }, chaseSpeed: 1, deaggroRadius: 4, leashRadius: { slime: 6 } },
        lod: { aoiRadius: 30 },
      }),
      hpFor: HP_FOR,
      attackStatsFor: ATTACK_STATS,
...
describe("mob AI leashRadius — per-mobType (D-055 §9.3) + aggro<leash invariant", () => {
  const ai = DEFAULT_ENGINE_CONFIG.mob.ai;

  test("leashRadius ต่อ mobType = §9.3 verbatim (slime 9 / bird 11 / boar 10 / elite 14 / boss 18)", () => {
    expect(ai.leashRadius.slime).toBe(9);
    expect(ai.leashRadius.bird).toBe(11);
    expect(ai.leashRadius.boar).toBe(10);
    expect(ai.leashRadius.boar_elite).toBe(14);
    expect(ai.leashRadius.boss_boiling_boar).toBe(18);
  });

  test("อ่านต่อ mobType จริง (ไม่ใช่ค่า global เดียว) — boss ≠ slime", () => {
    expect(ai.leashRadius.boss_boiling_boar).not.toBe(ai.leashRadius.slime);
  });

  test("invariant: leash > aggro ต่อ mobType ที่ระบุทั้งคู่ (acquire ก่อน leash-out; boss กัน kite-reset)", () => {
    // เดิม boss aggro 10 > leash 8 (global) = พังทั้ง invariant → boss leash-return ที่ 8 tile = kite-cheese.
    for (const mobType of Object.keys(ai.leashRadius)) {
      const aggro = ai.aggroRadius[mobType] ?? ai.defaultAggroRadius;
      const leash = ai.leashRadius[mobType];
      expect(leash).toBeGreaterThan(aggro);
    }
  });
});
