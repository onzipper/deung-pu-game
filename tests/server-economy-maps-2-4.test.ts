...
// Batch 5 — Maps 2–4 economy (MAPS_2_4_ECONOMY_AND_LOOT_SPEC §2/§4/§5) — never-downgrade zone (reward/RNG/ids).
// maps 2–4 = phase P2B (config พร้อม, ไม่ grant live จน owner promote §7) — mirror boss_map1_resonant_guardian.
...
describe("Maps 2–4 monster rewards (§2 identity/EXP/Gold)", () => {
  ...
  test("ครบ 15 monster + phase P2B (dormant จน owner promote)", () => {
    for (const id of MAP24_MONSTER_IDS) {
      const r = byId.get(id);
      expect(r, `monster reward missing: ${id}`).toBeDefined();
      expect(r!.phase, `${id} ต้อง P2B (OB = Map 1 only)`).toBe("P2B");
    }
  });
...
describe("Maps 2–4 drop tables (§5 well-formed)", () => {
  ...
  test("15 table (per-mob normal + elite + boss) ต่อ 3 แมพ + phase P2B", () => {
    expect(map24).toHaveLength(15);
    for (const t of map24) expect(t.phase, t.dropTableId).toBe("P2B");
  });
  ...
  test("ไม่มี upg_reinforcement/เศษ ในตารางไหน (boss-only via pity path §4.2/§3.5; boss M2–4 rate = owner-gated §7 Q3)", () => {
...
describe("EXP curve extension lv10–22 (§1 band · §7 Q1 owner-gated · Design Knob)", () => {
  ...
  test("live curve ยังคง levelCap 10 + 10 แถว (dormant, ไม่ wire) — ไม่กระทบ progression OB", () => {
    expect(live.levelCap).toBe(10);
    expect(live.levels).toHaveLength(10);
  });
