...
// export กลับมา" ของ resolver/label functions) — ไฟล์นี้สแกน **source text จริง** ทุกไฟล์ใต้
// src/ui/panels/bot/ ตรง ๆ (รวม .tsx component ที่มี JSX text node/string literal ตรง ๆ ในไฟล์ ไม่ใช่แค่ผ่าน
// helper function) กันกรณี copy ถูกพิมพ์ inline ใน component แทนที่จะผ่าน bot-view.ts.
//
...
const BOT_DIR = resolve(__dirname, "../src/ui/panels/bot");
...
describe("Bot Hub source copy-guard (M4) — สแกน source text ทุกไฟล์ใต้ src/ui/panels/bot/", () => {
  const files = collectSourceFiles(BOT_DIR);

  test("เจอไฟล์ source จริง (กัน false-pass ถ้า path เพี้ยน/ย้ายโฟลเดอร์)", () => {
    expect(files.length).toBeGreaterThan(0);
  });

  test.each(files.map((f) => [f.slice(BOT_DIR.length + 1).replace(/\\/g, "/"), f] as const))(
    "%s — ไม่มีคำต้องห้ามหลุดออกมานอก comment",
    (_relPath, file) => {
