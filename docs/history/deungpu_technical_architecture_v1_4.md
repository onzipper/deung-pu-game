# ดึ๋งปุ๊ — Technical Architecture v1.4

> **Canonical game spec:** `deungpu_project_checkpoint_v14_...ready.md` (รวมทุก layer: v9 combat + v10 audio + v11 tech handoff + v12 spawn + v13 engine + v14 runtime)
> **เอกสารแยกที่อ้างอิง:** `ENGINE_FOUNDATION_DECISIONS_v1` · `RUNTIME_BOT_CHANNEL_AND_SCHEMA_OWNERSHIP_DECISIONS_v1` · `MAP_LAYOUT_BIBLE_v1` · `MAP_SCALE_AND_SPAWN_DENSITY_SPEC_v1`
> สถานะ: **Decisions locked — เอกสารนี้คือ tech architecture ฝั่ง implementation; game semantics/balance ยึด v14**
> **v1.4 — sync กับ v14:** เพิ่ม isometric engine foundation (§17), spawn/aggro system (§18), runtime decisions (reconnect/offline bot/channel); ปรับ §15/§16 ให้ **reference** v14 §48/§50.1 แทน redefine (ตาม ownership rule)
...
>
> **Ownership boundary (v14 §59.4):** Design เป็นเจ้าของ skill fields/meaning/balance/guardrails (canonical: v14 §50.1 skill schema, §48 knobs). Tech (เอกสารนี้) เป็นเจ้าของ implementation/validation/persistence/runtime/performance. **เอกสารนี้ไม่ redefine field names — อ้างอิง §50.1 และอธิบายเฉพาะวิธี implement.** เปลี่ยน scope/spec ใดๆ ต้อง owner ยืนยันก่อน
...
| L5 | จำนวนอาชีพ (เดิม D5) | **5 อาชีพเต็มตาม spec ตั้งแต่ P0** (ดาบ/หอก/ธนู/เวท/อาคม) — ไม่ลด scope |
...
| **L14** | **Camera/Projection (v14 §57.1)** | **True 2D Isometric Pixel Art · diamond grid · fixed camera · no rotation** — frame-by-frame sprite |
| **L15** | **Direction (v14 §57.2)** | **5 ทิศวาด (S/SW/W/NW/N) + mirror (SE/E/NE)** · เผื่อ 8-dir override สำหรับ boss/NPC ในอนาคต |
| **L16** | **Map world model (v14 §57.3)** | **Separated map rooms/channels + loading/fade** · ไม่มี seamless open world ใน phase 1 |
| **L17** | **Runtime (v14 §59)** | Reconnect 30s grace → same pos / safe camp fallback · Offline bot = background worker (ไม่ materialize ใน public map) · Channel auto-assign + party sync |
...
- **Three.js** — ได้ perspective/กล้อง 3D จริง แต่เกมนี้เป็น sprite-based 2.5D **true isometric + fixed camera** (v14 §57.1) — จ่าย complexity 3D scene graph เพื่อสิ่งที่ 2D iso projection + depth sort ทำได้ = overkill
...
> ⚠️ v14 §57.1 ล็อก **True Isometric** — foundation layer นี้ต้องเขียนแบบ iso ตั้งแต่ P0 ไม่ใช่ทำ top-down ก่อนแล้วแปลง (จะเสียเวลาแปลง coordinate ซ้ำ) รายละเอียดเต็มดู **§17 Engine Foundation**
...
### Map instance / channel จัดการอย่างไร (v14 §57.3, §59.3)
...
**Online vs Offline (v14 §59.2 — ล็อกแล้ว):**
...
- Desktop กลางๆ: 60fps ที่ 40 mobs + 300 damage numbers/วินาที + 3 AoE ซ้อน
- มือถือกลาง (เช่น Android ระดับกลางอายุ 2–3 ปี): 30fps ที่ 30 mobs, quality Low
- ถ้า P0 ไม่ผ่าน budget นี้ → ลด pack size ใน design ก่อนจะฝืน optimize
...
|---|---|---|---|---|---|
| **P0 — Combat Feel** (local, ไม่มี server) | พิสูจน์ว่าเกม "สะใจ" จริงบน browser | **iso foundation (§17: projection/depth-sort/direction resolver 5+mirror)** + PixiJS scene: มอน 20–40, สกิล AoE, damage number, hit stop, shake, loot, quality switch, pooling | iso pipeline, pool architecture, BitmapText perf, **iso depth sort ถูกต้อง** | มือถือรับ horde ไหว?, GC pause, **iso coordinate/sort เพี้ยนไหม** | budget §11 ผ่าน desktop+mobile; iso render ถูกต้อง; คนลองเล่นว่า "มันส์" |
...
**กติกาเรียงเฟส:** P0 อยู่แรกเพราะ combat feel คือความมันหลักตาม design 18.6 — ถ้า P0 ไม่ผ่าน ไม่มีประโยชน์จะทำที่เหลือ และ **อย่าข้าม P2 ไป P4** — market ที่วางบน inventory ที่ dupe ได้คือหายนะ
...
**P0 Combat Feel** — มอน 40 ตัว + AoE 3 สกิล + damage number ถล่ม + hit stop/shake บน browser มือถือ ภายใน 2–3 สัปดาห์ นี่คือ bet หลักของเกม ถ้า feel ไม่ถึง ทุกระบบที่เหลือช่วยไม่ได้
...
> **Canonical values อยู่ที่ v14 §48 (Design Knobs) — design เป็นเจ้าของ.** ส่วนนี้บันทึกโครง combat ที่ owner เลือก (ชุดที่แนะนำ) และอธิบาย **วิธี implement** เท่านั้น ตัวเลขทุกตัวเป็น Design Knob อ่านจาก config (§10 versioned) ไม่ hardcode
> Field ที่เกี่ยวกับสกิลใช้ชื่อตาม **v14 §50.1** (เช่น `baseMultiplier`, `scalingStat`, `bossModifier`, `pvpModifier`) — เอกสารนี้ไม่ตั้งชื่อใหม่
...
- `baseMultiplier`, `scalingStat`, `bossModifier`, `pvpModifier` = field ตาม v14 §50.1 (design เป็นเจ้าของค่า)
...
- **`k` = Design Knob สำคัญที่สุด** (v14 §48 combat knobs) — ตัวเดียวปรับความ "อึด" ของทั้งเกม
...
### 16.1 Skill Data Model — Implementation of v14 §50.1
> **Canonical schema + field names = v14 §50.1 (design เป็นเจ้าของ).** เอกสารนี้อธิบายเฉพาะ **วิธี implement runtime/server/client/persistence** ของ 37 fields นั้น — **ไม่ rename ไม่ duplicate semantic field.** เพิ่ม field ใหม่ต้องผ่าน process §59.4 (เสนอ → design เคาะ → update §50.1 → tech implement → บันทึก migration)
...
**Reconnect: เคาะแล้ว (v14 §59.1)** — 30s grace → same pos / safe camp fallback; รายละเอียดใน §6 "Reconnect". **ยืนยัน tick rate จริงหลัง P0** (เสนอ 10Hz)
...
**Reference:** นโยบายลงโทษ/rollback/abuse boundary = **v14 §49** (design/owner เป็นเจ้าของ — ระบบไม่ auto-ban เอง). ตัวเลข rate limit จริงจูนจากพฤติกรรมตอนถึงระบบ
...
### 16.6 Design Knobs — Implementation (canonical = v14 §48)
> **Canonical knob list = v14 §48 (design เป็นเจ้าของ).** §48 มีครบแล้ว: Combat / Monster-Spawn / Drop-Reward / Enhancement / Economy / Bot / Hall-of-Fame knobs + Guardrail Principles. เอกสารนี้ระบุแค่ **วิธี implement**:
...
- Knob ที่เป็น owner/business ล้วน: **นโยบายลงโทษคนโกง** → v14 §49 (rollback-first, ไม่ auto-ban)
...
> Canonical deferred list + placeholder format = **v14 §51**. Tech สอดคล้อง: ยังไม่ออกแบบ data model จนกว่าจะมี combat/inventory/economy foundation (ออกแบบตอนนี้ = เดาทิ้ง)
...
## 17. Engine Foundation — Isometric (v14 §57 / ENGINE_FOUNDATION_DECISIONS_v1)

> ส่วนนี้คือ delta ใหญ่สุดจาก architecture เดิม — v14 §57.1 ล็อก **True Isometric** (ไม่ใช่ top-down 3/4 ที่เคยสมมติ) foundation layer ต้องเขียนแบบ iso ตั้งแต่ P0
...
1. **Update tech doc ให้ตรง v14** ✅ *เสร็จในรอบนี้ (v1.4)* — เพิ่ม iso engine foundation (§17), spawn/aggro (§18), runtime (reconnect/offline bot/channel), ปรับ §15/§16 เป็น reference v14 §48/§50.1
2. **UI Art Direction & Visual Design System** — เป็น **implementation** ของ v14 §45–47 (canonical UI direction/palette/screen mood = design เจ้าของ); Claude ทำ design token, component spec (ปุ่ม/panel/HUD/damage number), layout responsive 2 โหมด (PC keybind / touch โซนนิ้ว), **z-layer hierarchy** (ผูก priority mapping §16.5 + v14 §46 colors)
   - ⚠️ ขอบเขต: Claude ทำ **กรอบ+spec ที่ art จริงมาเติม** — ไม่ใช่ตัว visual art จริง (mockup/palette ลงตัว/ความรู้สึก Asian fantasy = งาน graphic ที่ต้องมีคนออกแบบภาพ)
3. **Skill runtime doc** — describe how v14 §50.1 fields run (ตาม §16.1) — **ไม่ redefine field**, อธิบาย loader + state machine (5-dir+mirror §17.4) + server calc + client render
4. **docs-as-memory ใน repo** — ตาม structure **v14 §52**: `/docs/tech/TECH_ARCHITECTURE.md` (เอกสารนี้), `/docs/design/*` (checkpoint + UI + audio + combat + knobs + skill + deferred), `/docs/prompts/*` (Claude Code + issue P0). Chat ไม่ใช่ source of truth — repo docs คือ ground truth; Claude Code อ่าน docs ก่อนทำงาน (v14 §53)
5. **Issue P0 + Claude Code prompt** — ท้ายสุด อ้างอิงทุกอย่างข้างบน + v14 §53 execution rules; **จะดีได้ต่อเมื่อ 1–4 นิ่งก่อน**

**P0 scope (ย่อ):** Combat Feel vertical slice บน **iso foundation** — 5 อาชีพ, มอน 20–40 + AoE + damage number + hit stop/shake + loot + quality switch + pooling, **placeholder ตาม iso grid L12 (proportion chibi ใกล้ mockup, 5-dir+mirror)**, local ไม่มี server. Success = budget §11 ผ่าน desktop+mobile + iso render ถูกต้อง + คนลองเล่นว่า "มันส์"
