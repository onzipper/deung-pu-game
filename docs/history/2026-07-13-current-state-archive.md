# Archive: current-state.md as of 2026-07-13 (pre agent-context-optimization)

> Verbatim copy taken before `docs/current-state.md` was restructured into a thin EN status board (chore/agent-context-optimization). Nothing below was edited.
...
1. **PR #10 (คลื่น 2) merge เข้า develop แล้ว (2026-07-12, owner เคาะ)** — ทดสอบผ่านครบทุกข้อ: รอบ 1 ข้อ 4-5 / รอบ 2 ข้อ 8 / review-fix 3 รอบ (สุดท้าย: fetch this-binding + hub key remount — ดู Latest work) แล้ว owner ยืนยันผ่าน 3 รายการสุดท้าย · branch `feat/p2-wave2` จบหน้าที่ (ยังไม่ลบ) · งานต่อไป = คลื่น 3 แตก branch ใหม่จาก develop
...
3. **E1/E2 RESOLVED, E3 OPEN (balance เท่านั้น) — owner เคาะ 2026-07-12**: ระบบตีบวกยกเครื่องเป็น **เสริมแกร่งการันตี** (เสริมแกร่ง ×1 → +1 สำเร็จ 100%, ไม่มี RNG/crack/repair/gold, เพดาน **+15 ทุกเฟส**, แหล่ง = **boss-only 8% + pity ถึงรอบ 15**) + **E1 บอส `boss_map1_resonant_guardian` / "ผู้พิทักษ์เสียงสะท้อน" / lv8 / P2B** · decision record + supersede marks + คำถามค้าง R1–R10 = **`docs/design/deungpu_REINFORCEMENT_SYSTEM_DECISION_v1.md`** (spec = game **v15.3**) → **P2-09/P2-10 เริ่มโครงได้** (transaction/UI/config/loader/flag; drop event จริงบน Map 1 = P2B) · **E3 = OPEN เฉพาะ production combat balance** (Pending Design Item "Map 1 Monster Combat Stat Table" — block production monster tuning + final combat QA เท่านั้น; schema/loader/placeholder/test **ไม่ block**) · ✅ **R1–R10 ปิดครบแล้ว (2026-07-13)** — สรุปผล: `TRADE_LIMITED` · ขายเฉพาะตลาดผู้เล่น (ไม่ขาย NPC) · flat 1 ชิ้น/ขั้น (+15 = long-term chase) · grant เดิมทุกจุด → Gold (จำนวน PENDING) · audio → P2B pass · P2 = UI inert + hint "ของหายากมากับบอส" · +6..+15 placeholder linear (PENDING, bucket E3) · เศษ = boss 10.7% independent roll (§3.5) · ยืนยัน `upg_reinforcement` rename ก่อน P2-16 — ดู decision-index 6 แถวล่างสุด + Reinforcement doc §11 · **ตัวเลข balance ที่ยังเปิด**: จำนวน Gold แทน grant · curve +6..+15 · E3 stat table (เคาะรวมตอน P2B)
4. **คลื่น 3 ถัดไป (หลัง merge PR #10)**: P2-07 inventory/equipment UI → P2-08 ledger → P2-09 drop+EXP+reinforcement config → P2-10 เสริมแกร่งการันตี → P2-11 ร้านค้า → P2-12 DG lite → P2-13 tab policy → P2-15 mobile → P2-17 storage/delivery + SVG-01 pipeline — spec ครบทุกเล่มแล้ว (UI/Storage/Economy/SVG-first/**Reinforcement** ใน docs/design/) · E3 เปิดเฉพาะ balance (ไม่ block โครง P2-09/10)
...
### Latest work (2026-07-12, branch `docs/reinforcement-e1-e3` — E1/E2/E3 decision record)
- **owner เคาะ E1 บอส + E2 เสริมแกร่งการันตี + E3 monster stat (open)** 2 ข้อความ (ข้อความที่ 2 ชนะ: เพดาน **+15** ไม่ใช่ 5, เสริมแกร่ง+เศษ **ขาย/trade ได้**) → เขียน **`docs/design/deungpu_REINFORCEMENT_SYSTEM_DECISION_v1.md`** (LOCKED, R1–R10) · supersede marks: **Economy 9 จุด** (top banner + scope 8–9 + §11 drop + §15 + §16 + §17 + §18.1 + upg_kraeng row + DoD) · **v15 → v15.3** (§0.0.2 amendment log + 7 banner: Success Rate/Failure, Item ตีบวก, World Boss drop, Raid drop, pillar 30–32, one-liner, §33 audio) · decision-index +4 แถว (E1/E2/E3/rename+trade+config) · P2 breakdown P2-09/P2-10 เขียนใหม่ (guaranteed, ตัด RNG/crack) + DoD ข้อ 9
- **จุดพบเพิ่มระหว่าง audit**: (1) milestone Kraeng grant มี **5 แถว** (ไม่ใช่ 2) + §15.2 source table + §11.6 first-kill → R5 · (2) Economy §15.3 เคย **ตัด fragment ออกจาก P2** ("No fragment decision") — ข้อความที่ 2 พลิกกลับ = re-scope → R7 · (3) **บอส = P2B ⇒ P2 ไม่มีแหล่งเสริมแกร่งบน Map 1 เลย** (ตีบวก inert ใน P2) → R8 · (4) §16.3 stat multiplier หยุดที่ +5 → +6..+15 ต้องเคาะ balance → R9 · (5) v15 lines World Boss/Raid drop = **rename อย่างเดียว** (ตรงทิศ hard-content philosophy) ไม่ใช่ supersede
- Gate: `npm test` **827/827 เขียว** (docs path-guard ผ่าน)
...
- **รอบ 3 (owner retest: ข้อ 8 ผ่าน, ข้อ 6/7 ยังพัง)** — บั๊กชั้นลึกกว่าเดิมทั้งคู่, root cause คนละที่กับรอบก่อน:
  - **ข้อ 6 — gate อัมพาตทั้งตัวใน browser จริง**: `GameCanvas` ส่ง `fetchFn: fetch` (unbound) → boot-gate เรียก `deps.fetchFn(...)` = this ผูก deps object → browser fetch brand-check โยน `TypeError: Illegal invocation` **ทุกครั้ง** → try/catch best-effort กลืน → คืน mount เสมอ (login อยู่ก็ไม่เด้ง /hub → เข้าเกม anonymous จุดเริ่มต้น). unit test เดิมผ่านหมดเพราะ Node/undici ไม่ brand-check this. Fix 2 ชั้น: caller ห่อ arrow `(input, init) => fetch(input, init)` + callee เรียกผ่าน local binding (`const doFetch = deps.fetchFn`) — จุดอื่นทั้ง codebase เป็น bare `fetch(...)` ปลอดภัย (grep ยืนยัน) · regression test จำลอง brand-check (`makeBrowserFetch`, fail ก่อนแก้/ผ่านหลังแก้) · trap ลง known-traps แล้ว
  - **ข้อ 7 — logout→login แล้วลิสต์ตัวละครว่างจนต้อง refresh**: `router.refresh()` re-run Server Component แต่ React preserve client instance → `useState(initialCharacters)` ใน HubShell ไม่ re-init. Fix: `hub/page.tsx` ผูก `key={session.accountId}` / `key="anon"` ให้ remount เมื่อ identity เปลี่ยน (mutate ใน session เดียวกัน/upgrade ไม่กระทบ) · trap ลง known-traps แล้ว
  - Gate (รันบน PowerShell หลัก): **827 tests เขียว** (+2 regression) · lint 0 errors · `next build` ผ่าน (ไม่แตะ server/) · e2e ไม่เพิ่มเพราะ harness รันบน Node = reproduce บั๊ก this-binding ไม่ได้ (unit + brand-check mock คือเครื่องมือที่ถูก)
