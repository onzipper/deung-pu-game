...
- P0-06 Player Sprite & Animation — แทน placeholder body/nose ด้วย sprite จริง 5-dir วาด + mirror (SE←SW, E←W, NE←NW); ใช้ `Direction` + `directionToScreenUnit` จาก `src/engine/movement/direction.ts` (logical resolver เสร็จแล้ว) map → texture/animation; idle/walk state ตาม intent (`docs/design/deungpu_P0_SCOPE_LOCK_v1.md` §4.5 · TA §17.4)
