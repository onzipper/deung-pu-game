...
**P0 เริ่มแล้ว** (Engine Foundation Vertical Slice, tech §19 · `docs/design/deungpu_P0_SCOPE_LOCK_v1.md`). **P0-01→P0-07 เสร็จ (รอ review)**. **P0-07 Realtime Room Skeleton เสร็จ**: Colyseus server แยก process (`server/`, รัน `npm run dev:server`) + client net layer (`src/engine/net/`) — join/leave · position sync ~12Hz · เห็น remote player ขยับ · channel stub (CH.1) · **offline-safe** (ไม่ start server = เล่น solo ได้). ถัดไป: **P0-08 Channel Stub / input attack** (ดู P0_SCOPE_LOCK §4.7+§4.9)
...
- **P0-08 Channel Stub / Dummy Mob Pocket** — channel stub foundation (mapId/roomId/channelId พร้อมแล้วใน room state จาก P0-07) + dummy mob spawn ใน pocket 2–3 จุด (`p0-test-field` มี pockets แล้ว). ดู P0_SCOPE_LOCK §4.7+§4.8 · TA §18
- **P0-09 Combat Stub** attack: manifest มี `attack` clip (loop=false) + placeholder texture พร้อม — เหลือ wire input/trigger
- **P0-11 Debug Overlay**: อ่าน `EngineHandle.net.status` (state/roomId/channelId/mapId/remoteCount) ที่ P0-07 เปิดไว้ + FPS/coord (P0_SCOPE_LOCK §4.10)
- ระวังตอนต่อ P1 World Sync: net layer P0 ยัง trust position + ไม่มี reconnect/validation — TODO ชี้ tech §6/§59.1 จดไว้ในโค้ด (`server/rooms/MapRoom.ts`, `net-client.ts`)
