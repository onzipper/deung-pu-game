...
1. **PR #10 (คลื่น 2) เปิดรอ owner review + ทดสอบ** — ลิสต์ทดสอบชุด A/B อยู่ใน PR body (https://github.com/onzipper/deung-pu-game/pull/10) · retest รอบ 2 (2026-07-12): **ข้อ 8 ผ่าน** (takeover เตะแท็บเก่า) · ข้อ 6/7 เจอบั๊กชั้นลึกกว่า → **review-fix รอบ 3 แก้แล้ว** (ดู Latest work) — ให้ owner retest: (ก) login แล้วปิดแท็บ → เข้า `/game` ตรง ๆ ต้อง**เด้งไป /hub** ไม่หลุดเข้าเกม anonymous, (ข) เข้าเกมผ่าน hub → map+ตำแหน่งเดิมครบ, (ค) logout ที่ hub แล้ว login กลับ → ลิสต์ตัวละครแสดงทันทีไม่ต้อง refresh
