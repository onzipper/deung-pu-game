...
## 0.0.5 Amendment Log — v15.4 (2026-07-15) — Character Autonomy + Dung-Dung/Help Boundary (D-067/D-068)

Owner ล็อกความหมาย Bot/Character Autonomy และดึ๋งๆ/Help ใหม่ 2026-07-15. decision records = `docs/decisions/D-067-character-autonomy.md` และ `docs/decisions/D-068-dungdung-contextual-guide.md` (Locked). Amendment นี้เป็น **CURRENT SOURCE OF TRUTH** สำหรับ Bot, tier, takeover, world presence, Report และความสัมพันธ์กับดึ๋งๆ/Help — เนื้อเดิมคงไว้เพื่อ history แต่ส่วนที่ระบุด้านล่างห้ามนำไป implement.

Delta:

1. **§4/§17.9/§19 ข้อ 8–12/§47.6–§47.7/§48.7** — **AMENDED โดย §4.1**: Bot Mode คือ Character Autonomy ของตัวละครจริงหนึ่งตัว; Auto Pilot เป็น Assisted auto-walk แยก; Report เป็น output. Tier ต่างด้วย continuity/recovery/workflow และทุก tier ใช้ combat/reward ceiling เดียวกัน
2. **§19 ข้อ 25 และ §57.7 ที่ให้ Bot opt-in risk zone** — **SUPERSEDED**: automation ห้ามเข้า boss, elite, event, secret, unsafe/risk หรือพื้นที่ที่ไม่อยู่ใน allowlist
3. **§59.1.1 เฉพาะ disconnect ขณะ Character Autonomy active, §59.2 ทั้ง section, §59.5 ข้อ 2, §60 และ §62 P2 scope ที่อ้าง offline worker/coarse simulation** — **SUPERSEDED**: client disconnect เป็น controller detach; actor จริงยัง materialized ใน real world/channel. ไม่มี clone, ghost/private bot, invisible worker หรือ offline reward simulation
...
> **AMENDED โดย §4.1 และ D-067 (2026-07-15):** เนื้อ §4 เดิมคงไว้เพื่อ history; mode grouping, tier behavior และ terminology ที่ขัดกับ §4.1 ห้ามนำไป implement
...
## 4.1 Amendment (v15.4, 2026-07-15) — Character Autonomy

### ความหมายและ authority

- **Manual:** ผู้เล่นควบคุมทั้งหมด
- **Assisted:** ระบบช่วยบางส่วน แต่ผู้เล่นยังคุมตัวละครหลัก; Auto Pilot ตาม D-037 อยู่ชั้นนี้
- **Character Autonomy:** ระบบควบคุมตัวละครจริงหนึ่งตัวตามแผน/เงื่อนไข ไม่มี clone, bot avatar หรือ worker entity
- **Report:** ผลสรุปจาก automation ไม่ใช่ mode
- manual movement/skill คืน authority ทันทีโดยไม่ต้องเปิดเมนู: หยุดคำสั่ง automation, checkpoint แผน และกัน input race ก่อนรับ manual intent

### ตัวละครจริงและโลกจริง

- ใช้ equipment, skill, HP, inventory, position และ progression จริง อยู่ใน world/channel จริง ผู้เล่นอื่นเห็นตัวละครจริงพร้อม automation status แบบไม่รบกวน
- ตัวละครถูกโจมตี ตาย ได้ EXP/loot และเสียทรัพยากรตามระบบปกติ; client disconnect ไม่สร้างหรือสลับ entity และ reconnect ต้อง attach ไป actor/state/position ล่าสุด
- actor ต้องนับ channel capacity และ automation population ต่อ farming pocket; ห้ามเข้า boss, elite, event, secret, unsafe/risk หรือพื้นที่นอก allowlist

### Tier และ lifecycle

| Tier | ความสามารถหลัก |
|---|---|
| Free | combat/loot พื้นฐานในพื้นที่เดียว; inventory/resource/death/stuck หลัง basic recovery/server restart → หยุด; report ล่าสุด |
| Plus | recovery routine ตาม rule/resources แล้วทำต่อ: potion/rest, sell/deposit/lock, repair/refill, **revive แล้ว return area หลังตาย**, change target, same-map fallback, schedule/notification/history เมื่อแผนและทรัพยากรอนุญาต; server restart → safe-stop |
...
- ทุก tier ใช้ damage, attack-speed rules, EXP rate, drop rate, loot luck และ combat efficiency ceiling เดียวกัน ไม่มี paid power/reward multiplier; manual expert ทำได้ดีกว่า automation
- profile/rule counts เป็น technical cap เท่านั้น คุณค่าหลักของ paid tier คือ continuity/recovery/workflow
- ordinary rare เป็น plan policy: keep+continue/notify/lock/deposit/stop; critical legendary/unique/item สำคัญมากอาจ global safe-stopตาม canonical item policy
...
> **AMENDED โดย §4.1/D-067:** plan/skill rules ทุก tierต้องอยู่ใต้ combat/reward ceiling เดียวกัน; elite/boss targeting และ paid combat optimization ที่ขัดกับ allowlist/no-power lock ห้าม implement
...
> **AMENDED โดย §0.0.5/§4.1:** ข้อ 8–12 ใช้ Manual→Assisted→Character Autonomy และ Report-as-output; ข้อ 25 ที่อนุญาต Bot opt-in risk zone ถูก SUPERSEDED
...
> **AMENDED โดย §4.1/D-067:** แยก Auto Pilot ออกจาก Character Autonomy และใช้ player-facing plan/takeover terminology
...
> **CLARIFIED โดย D-067:** Report เป็นผลสรุปจาก automation ไม่ใช่ mode
...
> **AMENDED โดย D-067:** knob ทุก tierอยู่ใต้ combat/reward ceilingเดียวกัน; rare action เป็น plan policy และ profile count เป็น technical cap
...
- auto restock limit
- sell filter
- ~~risk zone behavior~~ **SUPERSEDED โดย D-067:** automation ห้ามเข้า risk zone ทุก tier; ไม่มี opt-in knob
- ~~rare drop stop condition~~ ordinary rare ใช้ plan action; critical item ใช้ canonical global safe-stop policy
- Pro profile count
...
ครอบคลุม:
- reconnect behavior
- ~~offline bot materialization~~ **Character Autonomy materialization ตาม §4.1/D-067**
...
> **SUPERSEDED ทั้ง section โดย §0.0.5/§4.1/D-067 (2026-07-15):** ห้าม implement worker/coarse/ghost/private/invisible/offline reward simulation; Character Autonomy ควบคุม actor จริงใน real world/channel เท่านั้น
...
> **AMENDED โดย D-067:** actor ที่ automation ควบคุมต้องนับ channel capacity และ automation population ต่อ farming pocket
...
> **SUPERSEDED เฉพาะข้อ 2 Offline Bot โดย D-067:** ใช้ real-character materialization ตาม §4.1; ข้อ reconnect/channel/ownership ที่ไม่ขัดยังใช้ต่อ
...
> **SUPERSEDED เฉพาะข้อความ offline worker/coarse simulation โดย §0.0.5/D-067:** ขอบเขต phase อื่นคงเดิม
...
- ~~P1/P2 offline Pro bot ใช้ background worker simulation เป็น default~~ **SUPERSEDED โดย §0.0.5/D-067:** actor จริงอยู่ใน real world/channel ตลอด autonomy run
- ~~public map ไม่ต้อง materialize offline bot~~ **SUPERSEDED:** autonomous actor ต้อง visible/attackable และนับ capacity จริง
...
> **Historical v14 status (offline-bot clause SUPERSEDED โดย D-067):** v14 เคยปิด decision เพิ่มสำหรับ reconnect, offline bot, channel assignment และ skill/knob ownership โดยไม่เปลี่ยน foundation หลักของ v13
...
- P2 ค่อยทำ save/persistence/~~offline bot~~ **Character Autonomy foundation ตาม §4.1/D-067**/inventory/economy foundation
