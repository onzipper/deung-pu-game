...
| P2-12 Help + contextual guidance (current combined UI/follower behavior is noncanonical) | D-068 · DG spec §0.0 (Help static KB; Dung contextual/no persistent follower) | `src/ui/panels/help/`, `src/engine/player/companion.ts` | `tests/ui-panels-help-articles.test.ts`, `tests/ui-panels-help-rules.test.ts`, `tests/ui-panels-help-checklist.test.ts`, `tests/engine-player-companion-follow.test.ts` |
...
| Bot Character Autonomy + Report (implementation exists but clone/worker semantics are noncanonical; do not extend before alignment) | D-067 · GS §4.1 · P3 Bot UI §0.0 · Runtime Bot §0.0 · TA §0.0 | `server/bot/`, `server/rooms/MapRoom.ts`, `src/ui/panels/bot/` | `tests/server-bot-agent.test.ts`, `tests/server-bot-profiles.test.ts`, `tests/server-bot-reports.test.ts`, `tests/server-bot-tier.test.ts`, `tests/ui-panels-bot-view.test.ts` |
