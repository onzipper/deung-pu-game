...
- `src/ui/panels/` — panel framework (float/sheet + workspace, z-order, keydown; DG §13 + M4 `layout="workspace"` wide-window/full-screen variant) + `hud-icon-catalog.ts` (closed SVG icon set: 8 dock glyphs + bot/lock/town/warp/report/workflow/tier_free/tier_plus/tier_pro), provider in `src/ui/GameCanvas.tsx`
- `src/ui/panels/` subdirs — inventory/enhancement/shop/storage/help/mobile/settings · skillbar/status(StatusCluster dp tokens+class portrait fallback)/minimap(map-name+CH header)/auto-pilot(D-037)/world-status/dialogue(LW0 bark)/journal(C3 log+Achievement) · `bot/` (M4 workspace redesign, single "เริ่มบอท"/"หยุดบอท" CTA — 4 tabs ภาพรวม/แผนฟาร์ม/รายงาน/แพ็กเกจ id เดิม status/profiles/reports/packages; `bot-view.ts`+`bot-layout.ts` pure logic, `BotPanel.tsx` shell → `BotHubWindow.tsx` (header/tabs) → `tabs/*` → `editor/BotPlanEditor.tsx` (3-col desktop/stack mobile) + `editor/*Section.tsx`; `BotAlertToast.tsx`/`BotTakeoverToast.tsx`)
...
- `server/bot/` — real-actor controller; `server/bot/continuity.ts` = PR3 reducer/revision fence; `server/bot/policy.ts` + runtime = PR4 Free one-area/one-goal safe-stop settlement into WAITING_FOR_OWNER/COMPLETED/FAILED; `server/bot/recovery.ts` = PR5 recovery planner (D-073: Free = potions+floor only, no death/fallback); `server/bot/town-pressure.ts` = M2a proactive town-trip trigger (potion_low/bag_pressure/hp_no_potion, D-073); `server/bot/town-trip.ts` = PR5 town-trip warp (D-069/D-070) + D-071 M2b Free multi-hop walk (walk_out/walk_return loop per hop, takeover settles in place) + M2c `endAction:"stop_in_town"` (Plus goal `town_stop` parks in the city-hub + settles `goal_complete`); M2c runtime = Plus single-goal completion (`goalProgress`/`maybeHandleGoal`: safe_stop/notify_continue/town_stop/town_continue) + SELECTED_TYPES targeting (`pickTarget` allow-list) + live in-memory stats (`statsTimeBucket`); `server/bot/map-route.ts` = D-071 M2b pure BFS map-graph routing (buildMapAdjacency/planMapRoute/nextHopToward, built from the map registry, recomputed per hop); `server/bot/warp.ts` = shared synchronous actor transfer (PR6b, used by town-trip + workflow); `server/bot/workflow.ts` = PR6b Pro goal-chain engine (farm-goal→next / branch / town step / cross-map + fallbacks; contract `src/shared/bot-workflow.ts`); `server/bot/store.ts` `CheckpointRepo` + manager/runtime hooks = PR6a durable restart resume (D-067: Pro-only, write-behind, resume = new run re-validated)
...
- `prisma/migrations/` — 0001_init (13 tables) · 0002_shop_ledger_reasons · 0003_progression · 0004_bot (tier_state/profiles/sessions) · 0005_bot_checkpoint (durable restart resume, PR6a)

## src/shared + src/server (client↔server contracts + Next server-only)

- `src/shared/` — net-protocol + bot-continuity + bot-workflow (PR6b Pro goal-chain contract + pure validation/progress) wire contracts, reconnect/movement-validation, character validators, afk (pure)
