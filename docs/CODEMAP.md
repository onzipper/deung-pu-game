...
- `src/game/combat/damage-number.ts` — createDamageNumberLayer(): pixi glue ตัวเลข damage ลอย+fade เหนือ mob ผ่าน scene entity API (addEntity/removeEntity) — TODO(P1-06) BitmapText+pool (tech §11)
- `src/game/combat/combat-stub.ts` — createCombatStub(): client combat coordinator (**P1-05 server-authoritative**) — Space→cooldown gate→triggerAttack() anticipation ทันที→**online**: castSkill intent (net) + onSkillResult→damage number จาก **ผล server**; **offline**: dummy damage number (non-authoritative playground, ไม่ฆ่า) + hitbox debug wedge (shape สกิลจริง). **ไม่ import formula.ts** (server-only)
