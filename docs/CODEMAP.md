...
- `src/ui/panels/inventory/` — inventory/equipment panel (P2-07): inventory-view (pure logic) + InventoryPanel (bag grid + equip/unequip, no drag-drop) + InventoryHudButton ("I"). Reads HudState.inventory via the bridge; item name = raw itemId (client catalog = SVG-01). Also hosts the "เสริมแกร่ง" button.
- `src/ui/panels/enhancement/` — guaranteed-reinforcement panel (P2-10, Reinforcement §2.4): enhancement-view (8-state machine + R8 hint "ของหายากมากับบอส") + EnhancementPanel + EnhancementHudButton + enhancement-target-context (selected instanceId). **P2: server always rejects `NO_REINFORCEMENT` (R8/D-052) — inert by design, not a bug.**
- `src/ui/panels/shop/` — NPC shop panel (P2-11): shop-view (pure logic) + ShopPanel (buy/sell) + ShopHudButton (shows only when available). HudState shop/gold slices + net-client shop sends (per self-spawn).
- `src/ui/panels/help/` — Guidance "DG lite" (P2-12, client-only): help-articles (registry, oneLine≤120/steps≤4) + guidance-rules ("ทำอะไรต่อดี" v1, cooldown/dismiss, DG §7/§9) + tutorial-checklist (auto walk/kill/equip + manual skill) + guidance-preferences (localStorage) + HelpPanel/HelpHudButton/ContextHelpButton + help-focus-context.
