...
- `server/bot/` — real-actor controller; `server/bot/continuity.ts` = PR3 reducer/revision fence; `server/bot/policy.ts` + runtime = PR4 Free one-area/one-goal safe-stop settlement into WAITING_FOR_OWNER/COMPLETED/FAILED; `server/bot/recovery.ts` = PR5 recovery planner; `server/bot/town-trip.ts` = PR5 town-trip warp (D-069/D-070); `server/bot/store.ts` `CheckpointRepo` + manager/runtime hooks = PR6a durable restart resume (D-067: Pro-only, write-behind, resume = new run re-validated). PR6b workflow/UX pending
