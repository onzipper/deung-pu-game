...
3. automation intent ใช้ server-authoritative combat/economy/reward path เดียวกับ manual; tier ห้ามมี alternate damage/drop/EXP/luck/cadence path
4. room/channel allocation ต้องนับ autonomous actors และ per-pocket population; actorยัง visible/attackableและห้าม forbidden areas
5. durable run/checkpoint รองรับ Pro validated restart resume; Free/Plus safe-stop. Report เป็น projection/output ไม่ใช่ simulation authority
6. Worker/BullMQ ยังใช้กับ schedule dispatch, report projection, notification, telemetry/economy aggregation ได้ แต่ห้ามสร้าง gameplay resultแทน actorในโลก

## 0. Executive Summary

> **AMENDED โดย §0.0:** ประโยคที่จัด Bot combat/reward เป็น non-realtime background computation ถูก SUPERSEDED; report/ranking/economy aggregation ยังเป็น background jobได้
...
| **L17** | **Runtime (v15 §59)** | ~~Offline bot = background worker~~ **SUPERSEDED โดย D-067/v1.5.3** · Character Autonomy = real actor materializedใน real world/channel; reconnect active run attach actorเดิม; Channel auto-assign + party sync/capacity |
...
| Character Autonomy | **Server-authoritative real actor** | ~~Bot simulation = Server (worker) เท่านั้น~~ **SUPERSEDED โดย §0.0/D-067:** worker ไม่สร้าง combat/reward; automation ส่ง intent ให้ actor จริงใน world |
...
flowchart TB
  ...
  subgraph GameInfra["Render (SG, paid always-on) — long-lived Node"]
    ...
    %% SUPERSEDED D-067: historical "bot sim" worker label; worker must not create gameplay results
    WK["BullMQ Workers<br/>schedule / report projection /<br/>notification / telemetry rollup"]
  end
...
| Character Autonomy | ~~จำลองฟาร์มตาม profile/tierใน BullMQ worker (+materialize)~~ **SUPERSEDED โดย D-067:** automation intent ควบคุม actor จริงใน Colyseus world; workerทำ schedule/report/notification/telemetryเท่านั้น | **Colyseus authoritative actor** | bot_profiles, bot_runs (internal compatibility names) | **ใช่** | **สูง** — authority/takeover/recovery |
...
| Character Autonomy output | ~~คำนวณใน workerทั้งหมด~~ **SUPERSEDED โดย D-067:** combat/rewardเกิดจาก authoritative actor path เดียวกับ manual; worker project report เท่านั้น — client ส่ง plan/input intent |
...
characters 1─N bot_profiles             # internal compatibility name; plan workflow, shared-ceiling skill logic, recovery/item actions; risk opt-in SUPERSEDED/forbidden by D-067
bot_profiles 1─N bot_runs               # started_at, ends_at, map, state
bot_runs 1─1 bot_run_reports            # projection: kills, loot, gold, recovery/item actions, stop_reason; historical secret_hints field SUPERSEDED/forbidden for automation by D-067
...
> **AMENDED โดย §0.0/D-067:** worker/coarse/online-vs-offline/ghost-private architecture ใน section นี้ถูก SUPERSEDED. Retain เฉพาะ server authority, deterministic audit, report/telemetry และ no-power guardrailsที่ไม่จำลอง gameplay result
...
**Report:** authoritative actor events → workerประกอบ projection `bot_run_reports` (jsonb): kills ต่อ family, loot สำคัญ, gold curve, recovery/item actions และ stop reason → API อ่านตอน login ตาม Returning Flow. ~~เหตุการณ์ elite/risk/secret จาก automation~~ **SUPERSEDED/forbidden โดย D-067**
...
> **AMENDED โดย D-067:** P3 Bot milestone ต้องสร้าง real-character control/checkpoint/recovery/report architecture; BullMQ coarse simulation และ 12-hour job criteria ในตารางเดิมห้าม implement
...
- **แก้ start flow (2026-07-12, supersede Bible 3.2 บางส่วน — ดู decision-index)**: "Bot A intro แบบสั้น" **ถอดออกจาก tutorial เริ่มต้น** → เป็น optional micro tutorial 30–60 วิ ตอนผู้เล่นปลดล็อก/เปิด Bot A ครั้งแรก (ห้ามบังคับ) · เพิ่ม **first encounter "ดึ๋งๆ"** ที่ transition pocket ชาน starter district (ระหว่าง safe area กับทางออก Map 1 — เดินผ่านได้ ไม่ล็อกทาง กลับมาช่วยทีหลังได้; ตัว encounter+companion ทำใน P2B) — ระบบดึ๋งๆ ทั้งหมดยึด `docs/design/deungpu_DUNG_DUNG_COMPANION_GUIDE_SYSTEM_SPEC_v1.md` (locked design); P2 ทำเฉพาะ "DG lite" (help entry + เล่นยังไง + ทำอะไรต่อดี v1 — ไม่มีตัว companion) · Achievement/Journal ยึด `docs/design/deungpu_ACHIEVEMENT_AND_ADVENTURE_JOURNAL_SPEC_v1.md` (locked design) เริ่ม v1 ที่ P2B; **P2 ต้องเริ่มเก็บ GameEvent log** (append-only) เพื่อ retroactive credit + economy audit
...
**P2 excluded (อย่าลาก scope เข้า):** market/trade · ~~offline bot~~ Character Autonomy/report · guild/full party management · HoF/ranking · full quest graph · raid/world boss · การย้าย party ไป public shared channel model (§6.2 ข้อ 1)
...
> **AMENDED โดย D-067:** mitigation “bot = coarse sim” ถูก SUPERSEDED; ต้องคุม costด้วย channel/pocket admission, autonomous-room lifecycle, shared action path และ load telemetry
...
> **AMENDED โดย D-067:** worker-based Bot combat recommendation/ghost visibilityถูก SUPERSEDED; workerรับเฉพาะ non-gameplay orchestration/projectionตาม §0.0
...
> **AMENDED โดย D-067:** `botUsageRule` ใช้เป็น plan intentผ่าน actor action pathเดียวกับ manual ไม่ใช่ worker/materialized dual implementation และต้องอยู่ใต้ common combat ceiling
...
- **Sub-zone based** — safe camp ปิด PvP เสมอ, risk pocket เปิดตาม config, boss/event zone ตาม event rule, warning ก่อนเข้า, ~~bot opt-in เท่านั้น~~ **SUPERSEDED โดย §0.0/D-067: automation ห้ามเข้า boss/elite/event/secret/unsafe/risk ทุก tier**, ตายไม่เสียของ/เงิน/EXP/material
...
> **AMENDED โดย D-067:** autonomous actorใช้ spawnจริงและต้องนับ channel/pocket population; ห้าม boss/elite/event/secret/unsafe pockets
...
- Character Autonomy รวบได้ตาม plan ภายใต้ combat ceiling เดียวกัน; ~~elite ลาก pack เล็กตามได้~~ **SUPERSEDED โดย D-067: elite/boss/event/secret/risk ไม่เป็นเป้าหมาย automation**; boss add ไม่ถูกลากออก arenaเป็นกฎ manual combat
...
> **SUPERSEDED เฉพาะ offline bot worker simulation โดย D-067:** phase boundaryอื่นยังใช้ต่อ
...
- inventory
- item/gold save
- ~~offline bot worker simulation~~ **Character Autonomy foundation ตาม §0.0/D-067**
- report output
- market foundation
- progression save
...
7. **P2 preparation** — หลัง P1 ค่อยเริ่ม persistence/account/inventory/~~offline bot~~ **Character Autonomy ตาม §0.0/D-067**/market foundation
