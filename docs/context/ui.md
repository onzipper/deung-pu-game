...
- `src/ui/panels/bot/` — 4-tab Bot Hub. PR4 status/stopped messages carry server continuity through Free safe-stop settlement; the **PR7 UX** pass replaced the technical-profile-first presentation with continuity-first display: authority = `continuity.state` (14 states, `action` is fallback only); resume CTA splits on `checkpoint.kind` (takeover/restart) with an always-on "farmed results aren't lost" reassurance; workflow tab shows live step progress + a branch-step editor; terminology is "แผน/แผนงาน" everywhere (never "โปรไฟล์"), start = "มอบการควบคุม"; profile creation runs a 5-step wizard (map→pocket→preset→rules→stop policy); packages tab dropped the Schedule row (D-072); first-open panel shows a skippable 5-slide micro-tutorial (localStorage). Purchase MOCK (D-061); Bot ≠ Auto Pilot/Dung-Dung.
