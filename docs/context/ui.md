...
- `src/ui/panels/bot/` (7b-UI, P3 spec) — Bot Hub, 4 tabs (status/profiles/reports/packages; Schedule + Pro analytics = locked placeholder, deferred). bot-view.ts = pure logic (stop-reason map incl. 9 mandatory, rule/profile cap display, client mirror of server botAllowedPockets). HudState gained bot* slices (event-driven, pattern like shop/storage). BotHudButton ("B" hotkey); BotAlertToast = rare/captcha/gold_cap. Purchase = MOCK only (D-061); cross-tier overwrite = ConfirmDialog warning (D-063 §12.5). Never conflate with Auto Pilot (D-037) or the companion (D-035).
