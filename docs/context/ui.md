...
- `src/ui/panels/enhancement/` (P2-10, Reinforcement §2.4) — guaranteed-reinforcement panel: 8 UI states (`NO_ITEM / READY / PROCESSING / SUCCESS / NO_REINFORCEMENT / MAX_LEVEL / ITEM_LOCKED / UNKNOWN_RECONCILING`) driven by a pure state machine (`src/ui/panels/enhancement/enhancement-view.ts`, no client `maxLevel` knowledge — MAX_LEVEL only comes from the server's reject reason). Opens from InventoryPanel's "เสริมแกร่ง" button (sets the target via `src/ui/panels/enhancement/enhancement-target-context.tsx`, a small dedicated React Context — same "UI-only, not the Zustand bridge" rationale as PanelContext) or its own HUD button. HudState.enhanceResult (event-driven, `setEnhanceResult`) carries `MSG_ENHANCE_RESULT`; `sendEnhanceItem` on EngineHandle.net. **P2 always gets `NO_REINFORCEMENT` back (flag on, R8/D-052) — ship inert on purpose, not a bug** — copy "ของหายากมากับบอส" is locked verbatim (R8).
- `src/ui/panels/shop/` (P2-11) — NPC shop: buy tab (`MSG_SHOP_LIST` server prices) / sell tab (bag items, price only in `MSG_SHOP_RESULT`). shop-view.ts pure `ShopTxPhase` machine (mirrors enhancement) + qty clamp + Thai reject copy. HudState gained `shopList`/`shopResult`/`gold` (event-driven; `gold` also from `MSG_PLAYER_PROGRESS`, skips `GOLD_UNKNOWN`). ShopHudButton shows only when shopList available — `onSelfSpawn` re-requests it every join/map change. No walkable NPC yet (TODO → SVG-01). Confirm buttons = the "confirmation mandatory" invariant, non-modal like EnhancementPanel.
