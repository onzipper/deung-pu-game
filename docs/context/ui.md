...
- `src/ui/panels/inventory/` (P2-07) — bag grid + equipment list + equip/unequip buttons, HUD button + "I" hotkey. HudState gained an inventory/inventoryRejection slice, written **event-driven** (setInventoryState/setInventoryRejection from `src/engine/runtime/app.ts`'s net handlers on MSG_INVENTORY_STATE/MSG_INVENTORY_OP_REJECTED). Equip/unequip/move intents go via EngineHandle.net (imperative, not the store). No drag-drop (buttons only, mobile-first). Item display = raw itemId (no client catalog yet — SVG-01).
- `src/ui/panels/enhancement/` (P2-10, Reinforcement §2.4) — guaranteed-reinforcement panel: 8 UI states (`NO_ITEM / READY / PROCESSING / SUCCESS / NO_REINFORCEMENT / MAX_LEVEL / ITEM_LOCKED / UNKNOWN_RECONCILING`) via a pure state machine (enhancement-view.ts — no client `maxLevel`, MAX_LEVEL only from server reject reason). Opens from InventoryPanel's "เสริมแกร่ง" button (target via enhancement-target-context.tsx, same rationale as PanelContext) or its own HUD button. HudState.enhanceResult (event-driven) carries `MSG_ENHANCE_RESULT`. **P2 always gets `NO_REINFORCEMENT` back (flag on, R8/D-052) — inert on purpose, not a bug** — hint "ของหายากมากับบอส" locked verbatim.
