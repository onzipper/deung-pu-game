...
## Traps
- **self-authority must interpolate, never snap-per-patch (M6)** — Symptom: screen + sprite stutter (freeze ~6 frames then jump) while a bot drives your own actor. Cause: server sends self-state ~10Hz; snapping position + camera on every patch = 60fps screen jumps between sparse updates. Rule: during Character Autonomy the local player pushes authority state into a self interpolation buffer (`src/engine/net/self-authority.ts`, reuses `interpolation.ts`) and samples per-frame at `now − bufferMs` with a **lerped** camera — same as remote players. Only seed / warp (jump ≥ `net.selfAuthoritySnapThresholdTiles`) / end-of-autonomy flush snap once. Never revert `local-player.applyAuthorityState` to snap-every-patch. `applyCorrection` stays snap-always (server truth) and manual prediction path stays byte-identical.
